...
#     """
#     Quét bảng đáp án thông minh:
#     - Hỗ trợ dạng cặp dọc/ngang: | 1 | A |
#     - Hỗ trợ dạng hàng ngang (Matrix):
#       | 1 | 2 | 3 |
#       | A | C | B |
#     """
#     answers_map = {}
...
#         # --- CHIẾN THUẬT 1: QUÉT THEO HÀNG NGANG (Row-based) ---
#         # Dành cho bảng dạng:
#         # Row 1: 1, 2, 3
#         # Row 2: A, B, C
#         rows = table.rows
...
#             row_a = rows[r+1]
#
#             # Logic: Kiểm tra nếu hàng trên toàn số, hàng dưới toàn chữ A-D
#             # và số lượng ô khớp nhau
#             valid_pairs = 0
#             temp_row_map = {}
#
#             # Chỉ xét khi số lượng ô tương đương (cho phép lệch chút do merge cell)
...
#                         temp_row_map[q_num] = ans
#                         valid_pairs += 1
#                     except: pass
#
#             # Nếu tìm thấy >= 3 cặp khớp trong 2 hàng này -> Chắc chắn là bảng đáp án
#             if valid_pairs >= 3:
#                 answers_map.update(temp_row_map)
#                 continue # Đã quét xong cặp hàng này, chuyển tiếp
#
#         # --- CHIẾN THUẬT 2: QUÉT CẶP LIỀN KỀ (Cell-based) ---
#         # Dành cho dạng | 1 | A | 2 | B | (Fallback)
#         # Chỉ chạy nếu chiến thuật 1 chưa tìm thấy gì nhiều
...
#                 try:
#                     q_num = int(q_match.group(1))
#                     # Chỉ cập nhật nếu chưa có (ưu tiên Chiến thuật 1 chính xác hơn)
...
"""
Core logic V11.0: Structure Preservation & Answer Key Removal
- Auto-detects and removes Answer Key tables/sections.
- Supports Section headers like "I. Trắc nghiệm", "PHẦN 1...".
- Normalizes output format (Bold labels, Unbold content).
"""
from __future__ import annotations
import io
import random
...
def _parse_questions_in_range(blocks: List[Tuple[str, object]]) -> List[QuestionBlock]:
    ...
    for i, start in enumerate(q_indices):
        next_q_start = q_indices[i + 1] if i + 1 < len(q_indices) else len(blocks)
        raw_chunk = blocks[start:next_q_start]
        # Không cắt footer ở đây nữa, footer cắt ở tầng cao nhất
...
def parse_exam_template(source_bytes: bytes) -> ExamStructure:
    ...
    # 5. Merge đáp án từ bảng vào câu hỏi
    if table_answers:
        for sec in structure.sections:
            for q in sec.questions:
                if q.original_idx in table_answers:
