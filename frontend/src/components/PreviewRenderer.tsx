...
interface QuestionData {
  ...
  contentLines: string[];
  uppercaseAnswers: { letter: string; content: string; isCorrect: boolean }[];
  lowercaseAnswers: { letter: string; content: string; isCorrect: boolean }[];
      ...
      while (i < lines.length) {
        ...
        if (currentPart !== 3 && hasUppercaseAnswerPattern(currTrimmed)) {
          ...
          extracted.forEach(a => upAnswers.push({
            letter: a.letter, content: a.content, isCorrect: a.isMarkedCorrect
          }));
          i++;
        ...
        <div className="answer-options space-y-2">
          {uppercaseAnswers.map((answer) => (
            <MultipleChoiceOption
              ...
              onClick={() => onAnswerSelect?.(questionIndex, answer.letter)}
        ...
        <div className="answer-options space-y-2">
          {lowercaseAnswers.map((answer) => (
            <TrueFalseOption
              ...
              onClick={() => onTrueFalseToggle?.(questionIndex, answer.letter)}
              parseTokens={(text) => parseTokens(text, assetsMap)}
            />
          ))}
        </div>
