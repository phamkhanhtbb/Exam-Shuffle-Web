...
class MathProcessor:
    ...
    def process_omml_element(self, element) -> Optional[str]:
        ...
        try:
            ...
            if not latex:
                logger.warning(
                    f"omml_to_latex returned empty/None for: {omml_xml[:100]}..."
                )
            return latex
    ...
    def extract_latex_from_run(self, run_element) -> Optional[str]:
        ...
        if not omath_elements:
            ...
            if omath_para:
                omath_elements = omath_para[0].findall(
                    ".//m:oMath", namespaces=self.nsmap
                )
        ...
        alt_omath = run_element.findall(
            ".//mc:Choice//m:oMath", namespaces={**self.nsmap, **alt_content_ns}
        )
        if alt_omath:
    ...
    def extract_ole_image_bytes(
        self, obj_element, get_image_data_callback
