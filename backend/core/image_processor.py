...
class ImageProcessor:
    ...
    def _try_convert_wmf_emf(
        self, img_bytes: bytes, fmt: str, img_id_for_log: str
    ) -> str:
        ...
        if HAS_WIN32 and fmt in ("wmf", "emf"):
            try:
                ...
                try:
                    ...
                    if fmt == "wmf":
                        ...
                        hmf = ctypes.windll.gdi32.SetWinMetaFileBits(
                            len(data_bytes), data_bytes, None, None
                        )
                    else:
                        # SetEnhMetaFileBits
                    ...
                    print(
                        f"[DEBUG] {img_id_for_log}: GDI loading failed: {load_err}, trying temp file"
                    )
                ...
                res = ctypes.windll.gdi32.GetEnhMetaFileHeader(
                    hmf, ctypes.sizeof(header), ctypes.byref(header)
                )
                ...
                print(
                    f"[DEBUG] {img_id_for_log}: pywin32 conversion failed ({fmt}): {e}"
                )
        ...
        if Image:
            try:
                ...
                print(
                    f"[DEBUG] {img_id_for_log}: Pillow conversion also failed ({fmt}): {e}"
                )
