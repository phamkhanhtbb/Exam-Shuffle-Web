...
class InvalidExamFormatException(ExamError):
    ...
    def __init__(
        self, message="Định dạng đề thi không hợp lệ. Vui lòng kiểm tra lại cấu trúc."
    ):
...
class AnswerKeyNotFoundError(ExamError):
    ...
    def __init__(
        self,
        message="Không tìm thấy bảng đáp án hoặc từ khóa 'ĐÁP ÁN'. Vui lòng kiểm tra lại file.",
    ):
...
class EmptyQuestionError(ExamError):
    ...
    def __init__(
        self,
        message="Không tìm thấy câu hỏi nào trong đề thi. Vui lòng kiểm tra lại các từ khóa 'Câu', 'Bài'.",
    ):
